...
//In questo caso aspetto che il figlio termini. Questo dovrebbe essere concesso dall'esercizio dato che l'ordinamento fatto dal padre viene eseguito prima di aspettare la terminazione del figlio
//Infatti qui aspettiamo che anche il figlio termini per verificare che effettivamente sia stato eseguito l'ordinamento da entrambi in modo corretto
  int e;
  xwait(&e,__LINE__,__FILE__);
