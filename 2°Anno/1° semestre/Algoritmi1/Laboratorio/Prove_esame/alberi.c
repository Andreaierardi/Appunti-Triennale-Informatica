...
Nodo *makeTree(int n, int radice) //Creazione di un albero passando il numero di nodi da creare
{
     ...
     if (n >0 )
    {
        ...
        nuovoNodo->sinistro =makeTree(numFigliSX,0); //Richiamo la funzione maketree sul ramo sinistro per la generazione dei nodi di ogni figlio
        nuovoNodo->destro = makeTree(numFigliDX,0); //Idem per il figlio destro
...
bool radiceMax(Nodo *r, int radix);
int maxAlbero(Nodo *r);
int countV(Nodo *r , int V);
///Funzione main
int main()
{

    Nodo *Albero = makeTree(10,1); //Creazione di un albero di 10 nodi --> con valore interno casuale
///1)
...
///5b)
    ...
    if (radiceMax(Albero,Albero->inf))
        printf("Radice è il massimo : %d\n",Albero->inf);
    else
        printf("Radice non è il massimo: %d\n",Albero->inf);

    int max = maxAlbero(Albero);
    int count = countV(Albero,5);

    printf("Massimo : %d\nTrovati nodi con val %d: %d\n", max,3,count);
...
bool radiceMax(Nodo *r, int radix)
{
    if (r!=NULL)
    {
        if (r->inf > radix)
            return false;
        return radiceMax(r->destro,radix);
        return radiceMax(r->sinistro,radix);
...
int maxAlbero(Nodo *r)
{
    ...
    int sx = maxAlbero(r->sinistro);
    if(r->inf > sx)
        sx = r->inf;
    int dx = maxAlbero(r->destro);
    if(r->inf > dx)
        dx = r->inf;
    return max(sx,dx);
}
...
int countV(Nodo *r , int V)
{
    if (r== NULL) return 0;
    if (r->inf <= V)
        return 1+countV(r->sinistro,V)+countV(r->destro,V);
    return countV(r->sinistro,V)+countV(r->destro,V);
}
