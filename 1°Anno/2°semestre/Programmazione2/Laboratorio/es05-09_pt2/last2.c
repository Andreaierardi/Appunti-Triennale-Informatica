...
void unions(LINK lis, LINK lis2, LINK *head,LINK *tail);
...
void unions(LINK lis, LINK lis2, LINK *head,LINK *tail)
{
    ...
    if(lis!=NULL)
    {
        ...
        if (lis2!=NULL)
        {
            ...
            if((strcmp(lis->d.autore,lis2->d.autore)==0)&&(strcmp(lis->d.titolo,lis2->d.titolo)==0))
            {
                p=newnode();
                ...
                p->next=NULL;
                unions(lis,lis2->next,head,tail);

                if(*head==NULL) {*head=p; *tail=p;}
                else {(*tail)->next=p; *tail=p;}
            }

            else {unions(lis->next,testa2,head, tail);}
